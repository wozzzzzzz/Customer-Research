...
import Image1 from './components/망개.png';
import Image2 from './components/굴레.png';
import Image3 from './components/사과.png';
import Image4 from './components/수제.png';
import Image5 from './components/흑임자.png';
import Image6 from './components/밥알.png';
import Image7 from './components/오메기.png';
...
const Subpage4 = () => {
    const [answers, setAnswers] = useState({
        OneTimeConsumptionCount: '',
        FavoriteRiceCake: '',
        ConsumptionSatisfaction: '',  
        SatisfactionReason:'',
        StorageMethod: '',  
    });
        ...
        if (user) {
            try {
                ...
                if (!querySnapshot.empty) {
                    ...
                    if (answers.StorageMethod === '냉동 보관') {
                        navigate('/Subpage5'); // Navigate to Subpage5 for 선물용
                    } else if (answers.StorageMethod === '실온 보관' || answers.StorageMethod === '냉장 보관') {
                        navigate('/Subpage6'); // Navigate to Subpage6 for 섭취용
                    }
            ...
            <div style={{
                ...
                <div style={containerStyle}>
                    ...
                    <div style={buttonGroupStyle}>
                            {['1개', '2개', '3개', '4개이상'].map((range) => (
                                <button
                                    type="button"
                                    key={range}
                                    style={answers.OneTimeConsumptionCount === range ? selectedButtonStyle : buttonStyle}
                                    onClick={() => handleChange('OneTimeConsumptionCount', range)}>
                                    {range}
                                </button>))}
                        </div>

                    <div style={labelStyle}>
                        망개랑 떡 중<br/> 제일 좋아하는 떡은 무엇인가요?
                    </div>

                    <div style={buttonGroupStyle}>
                        {['망개떡', '굴레떡','사과말이떡','수제찹쌀떡','흑임자말이','밥알쑥찰떡','오메기떡',].map((range) => (
                            <button
                            type="button"
                            key={range}
                            style={{
                              ...buttonStyle2,
                              backgroundColor: answers.FavoriteRiceCak === range ? '#7BD494' : '#E3F9D3',
                              color: answers.FavoriteRiceCak === range ? 'white' : '#7BD494',
                            }}
                            onClick={() => handleChange('FavoriteRiceCak', range)}>
                            {range === '망개떡' ? (
                              <img src={Image1} alt="Image1" /> // PNG 이미지 사용
                            ) : range === '굴레떡' ? (
                              <img src={Image2} alt="Image2" /> // 추가 이미지 사용
                            ) : range === '사과말이떡' ? (
                              <img src={Image3} alt="Image3" /> // 추가 이미지 사용
                            ) : range === '수제찹쌀떡' ? (
                              <img src={Image4} alt="Image4" /> // 추가 이미지 사용
                            ) : range === '흑임자말이' ? (
                              <img src={Image5} alt="Image5" style={imageStyle} /> // 추가 이미지 사용
                            ) : range === '밥알쑥찰떡' ? (
                              <img src={Image6} alt="Image6" style={imageStyle} /> // 추가 이미지 사용
                            ) : (
                              <img src={Image7} alt="Image7" style={imageStyle}/> // 추가 이미지 사용
                            )}
                            {range}
                          </button>
                        ))}
                    ...
                    <div style={labelStyle}>
                        망개랑 떡을 5점 만점으로 평가한다면 몇 점을 주시겠어요?
                    </div>
                    ...
                    <div style={buttonGroupStyle}>
                        {[1, 2, 3, 4, 5].map((value) => (
                            <button
                                ...
                                style={{
                                    ...buttonStyle,
                                    backgroundColor: answers.ConsumptionSatisfaction === value ? '#7BD494' : '#E3F9D3',
                                    color: answers.ConsumptionSatisfaction === value ? 'white' : '#7BD494',
                                }}
                                onClick={() => handleChange('ConsumptionSatisfaction', value)}>
                            {value}
                            </button>))}
                        </div>
                    ...
                    <label style={labelStyle}>
                            그 점수를 주신 이유는 무엇인가요?
                            <textarea
                                value={answers.SatisfactionReason}
                                onChange={(e) => handleChange('SatisfactionReason', e.target.value)}
                                placeholder="해당 점수를 주신 이유를 입력하세요"
                                style={inputStyle}
                            />
                        </label>

                    <div style={labelStyle2}>
                        떡을 보관해야 할 때 어떤 방법으로 보관하시나요?
                    </div>


                    <div style={buttonGroupStyle}>
                            {['실온 보관', '냉동 보관', '냉장 보관'].map((range) => (
                                <button
                                    type="button"
                                    key={range}
                                    style={answers.StorageMethod === range ? selectedButtonStyle : buttonStyle}
                                    onClick={() => handleChange('StorageMethod', range)}>
                                    {range}
                                </button>))}
                        </div>
                        <button
